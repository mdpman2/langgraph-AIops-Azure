# LangGraph Style AI Agent Framework for AIops v2.0

Azure SDK 및 Microsoft Agent Framework를 기반으로 구현한 LangGraph 스타일의 AI 에이전트 프레임워크입니다.

[![Tests](https://img.shields.io/badge/tests-71%20passed-brightgreen)](tests/) [![Python](https://img.shields.io/badge/python-3.11+-blue)](https://python.org) [![Azure](https://img.shields.io/badge/Azure-Container%20Apps-0078D4)](https://azure.microsoft.com) [![GPT-5.2](https://img.shields.io/badge/GPT--5.2-model--router-green)](https://learn.microsoft.com/azure/ai-foundry) [![Version](https://img.shields.io/badge/version-v2.0.1-orange)](CHANGELOG.md)

> **📢 2026-01-26 업데이트**: GPT-5.2 via model-router, Structured Outputs, Agent Evaluators, 성능 최적화 (테스트 40% 단축)

### 주요 기능
- 🎨 **웹 기반 채팅 UI**: 직관적인 대화형 인터페이스
- ⚡ **실시간 스트리밍**: SSE(Server-Sent Events) 기반 응답 스트리밍
- 🔄 **단계별 진행 표시**: Planning → Execution → Reflection → Decision 과정 실시간 확인
- ☁️ **Azure Container Apps**: 자동 스케일링 및 고가용성
- 🧠 **GPT-5.2 Reasoning**: 고급 추론 및 Structured Outputs 지원
- 📊 **Agent Evaluators**: 의도 파악, 도구 호출, 작업 준수도 평가
- 🚀 **성능 최적화**: frozenset O(1) 검색, __slots__ 메모리 절감, 비동기 컨텍스트 매니저

---

## 🎯 AIops 한눈에 보기

> **AIops** = AI 에이전트 개발부터 평가, 배포, 모니터링까지 전체 라이프사이클 자동화

### 📌 AIops 3단계 요약

```
┌─────────────────────────────────────────────────────────────────────────────┐
│  STAGE 1: 에이전트 개발 (Local)                                              │
│  ─────────────────────────────────────────────────────────────────────────   │
│  Planning → Execution → Reflection → Decision 사이클 구현                    │
│  • Azure AI Foundry GPT-5.2 via model-router                                │
│  • Structured Outputs + reasoning_effort 지원                               │
│  • 자기 반성 및 재계획 루프                                                  │
└─────────────────────────────────────────────────────────────────────────────┘
                                    │
                                    ▼
┌─────────────────────────────────────────────────────────────────────────────┐
│  STAGE 2: CI/CD 파이프라인 (GitHub Actions)                                  │
│  ─────────────────────────────────────────────────────────────────────────   │
│  코드 Push → 테스트 → 평가 → Docker 빌드 → Canary 배포                       │
│  • Azure AI Evaluation SDK로 품질 평가                                       │
│  • 평가 통과 시 자동 배포                                                    │
└─────────────────────────────────────────────────────────────────────────────┘
                                    │
                                    ▼
┌─────────────────────────────────────────────────────────────────────────────┐
│  STAGE 3: 프로덕션 운영 (Azure)                                              │
│  ─────────────────────────────────────────────────────────────────────────   │
│  Azure Container Apps에서 에이전트 실행 + 모니터링                           │
│  • Application Insights로 추적                                               │
│  • Cosmos DB에 대화 기록 저장                                                │
└─────────────────────────────────────────────────────────────────────────────┘
```

### 🚀 5분 만에 AIops 실행하기

```bash
# 1️⃣ 로컬에서 에이전트 실행
pip install -e .
python -m langgraph_agent "Python으로 REST API 만드는 방법 알려줘"

# 2️⃣ 품질 평가 실행
python simple_eval.py

# 3️⃣ Azure에 배포 (대화형 모드 - 권장)
python scripts/deploy.py --interactive

# 또는 전체 워크플로우 실행 (평가 → 배포)
python scripts/agent_eval_deploy.py --skip-agent
```

### 📊 AIops 품질 게이트 (자동 평가)

#### 기본 품질 지표
| 평가 지표 | 설명 | 통과 기준 |
|-----------|------|------------|
| 🎯 **Groundedness** | 응답이 사실에 기반하는가? | ≥ 0.7 |
| 🔗 **Relevance** | 질문과 관련 있는 답변인가? | ≥ 0.7 |
| 📝 **Coherence** | 논리적으로 일관성 있는가? | ≥ 0.8 |
| 💬 **Fluency** | 자연스럽게 표현되었는가? | ≥ 0.8 |

#### 🤖 Agent Evaluators (2026 최신)
| 평가 지표 | 설명 | 통과 기준 |
|-----------|------|------------|
| 📍 **Intent Resolution** | 에이전트가 사용자 의도를 정확히 파악했는가? | ≥ 0.6 (3/5) |
| 🛠️ **Tool Call Accuracy** | 적절한 도구를 올바른 파라미터로 호출했는가? | ≥ 0.6 (3/5) |
| 📋 **Task Adherence** | 에이전트가 지시된 작업을 준수했는가? | ≥ 0.6 (3/5) |

> ⚠️ **품질 게이트**: 모든 지표가 기준을 통과해야만 배포됩니다!

---

## 🏗️ 아키텍처

```
┌─────────────────────────────────────────────────────────────┐
│                    User Request                              │
└─────────────────────────────────────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────┐
│                     Planning Node                            │
│  - 사용자 요청 분석                                          │
│  - 실행 계획 수립 (PlanStep 리스트)                          │
│  - 목표 및 예상 결과 정의                                    │
└─────────────────────────────────────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────┐
│                    Execution Node                            │
│  - 계획된 단계 순차 실행                                     │
│  - 도구 호출 및 LLM 추론                                     │
│  - 실행 결과 수집                                            │
└─────────────────────────────────────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────┐
│                   Reflection Node                            │
│  - 실행 결과 품질 평가                                       │
│  - 목표 달성도 측정                                          │
│  - 개선점 및 제안 도출                                       │
└─────────────────────────────────────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────┐
│                    Decision Node                             │
│  - COMPLETE: 작업 완료                                       │
│  - CONTINUE: 다음 단계 진행                                  │
│  - RETRY: 현재 단계 재시도                                   │
│  - REPLAN: 계획 재수립                                       │
└─────────────────────────────────────────────────────────────┘
                              │
              ┌───────────────┼───────────────┐
              │               │               │
              ▼               ▼               ▼
         [COMPLETE]    [CONTINUE/RETRY]   [REPLAN]
              │               │               │
              ▼               │               │
         Final Output    ────┴───────────────┘
                              │
                              ▼
                        (Loop back)
```

## 📁 프로젝트 구조

```
langgraph_agent_framework/
├── src/
│   └── langgraph_agent/
│       ├── __init__.py          # 패키지 초기화
│       ├── config.py            # 설정 관리 (pydantic-settings)
│       ├── models.py            # 데이터 모델 정의
│       ├── workflow.py          # 워크플로우 구현 (핵심)
│       ├── server.py            # 🆕 FastAPI 웹 서버 (REST API + SSE 스트리밍)
│       ├── main.py              # CLI 진입점
│       └── static/
│           └── index.html       # 🆕 웹 채팅 UI
├── tests/
│   ├── evaluation/
│   │   ├── run_evaluation.py    # 품질 평가 실행
│   │   ├── check_thresholds.py  # 임계값 검증
│   │   ├── test_agent_behavior.py # 행동 테스트
│   │   └── test_cases.jsonl     # 테스트 케이스
│   ├── integration/
│   │   └── test_api.py          # 🆕 API 통합 테스트
│   └── unit/
│       ├── test_workflow.py     # 단위 테스트
│       └── test_deploy.py       # 🆕 배포 스크립트 테스트
├── scripts/
│   ├── agent_eval_deploy.py     # 🆕 평가 → 배포 통합 워크플로우
│   ├── deploy.py                # 🆕 자동 배포 (재시도, 헬스체크 포함)
│   ├── deploy_to_azure.py       # 새 Azure OpenAI 포함 배포
│   └── check_region_availability.py  # 지역 가용성 확인
├── infra/
│   ├── main.bicep               # Azure 인프라 정의 (전체)
│   ├── main.bicepparam          # 배포 파라미터
│   ├── deploy-with-existing-aoai.bicep  # 🆕 기존 OpenAI 사용
│   └── main-gpt41.bicep         # GPT-4.1 전용 배포
├── .github/
│   └── workflows/
│       └── ci-cd.yml            # GitHub Actions CI/CD
├── Dockerfile                   # Docker 이미지 정의 (멀티스테이지 빌드)
├── docker-compose.yml           # 로컬 개발 환경
├── requirements.txt             # Python 의존성
├── pyproject.toml               # 패키지 설정
├── pytest.ini                   # 테스트 설정
└── README.md                    # 이 문서
```

## 🚀 빠른 시작

### 1. 환경 설정

```bash
# 저장소 클론
cd langgraph_agent_framework

# 가상환경 생성
python -m venv .venv
.venv\Scripts\activate  # Windows
# source .venv/bin/activate  # Linux/Mac

# 의존성 설치
pip install -r requirements.txt
```

### 2. 환경 변수 설정

`.env.template`를 `.env`로 복사하고 설정:

```bash
cp .env.template .env
```

필수 환경 변수:
```env
# Azure AI Foundry (권장)
AZURE_FOUNDRY_PROJECT_ENDPOINT=https://your-project.ai.azure.com
AZURE_FOUNDRY_MODEL_DEPLOYMENT=gpt-4.1

# 또는 Azure OpenAI
AZURE_OPENAI_ENDPOINT=https://your-resource.openai.azure.com
AZURE_OPENAI_API_KEY=your-api-key
AZURE_OPENAI_DEPLOYMENT_NAME=gpt-4.1
```

### 3. 패키지 설치 및 실행

```bash
# 패키지 설치 (필수 - 최초 1회)
pip install -e .

# CLI로 실행
python -m langgraph_agent.main

# 웹 서버로 실행 (UI 포함)
python -m uvicorn langgraph_agent.server:app --host 0.0.0.0 --port 8080

# 또는 설치 후
pip install -e .
langgraph-agent
```

### 4. 웹 UI 접속

웹 서버 실행 후 브라우저에서 http://localhost:8080 접속

**API 엔드포인트:**
- `GET /` - 웹 채팅 UI
- `GET /health` - 헬스체크
- `POST /agent` - 에이전트 실행 (JSON 응답)
- `POST /agent/stream` - 에이전트 실행 (SSE 스트리밍)

## 🔧 개발

### 테스트 실행

```bash
# 모든 테스트 (71개)
pytest

# 단위 테스트만
pytest tests/unit -v

# 평가 테스트
pytest tests/evaluation -v

# 통합 테스트 (API 엔드포인트)
pytest tests/integration -v

# 커버리지 포함
pytest --cov=langgraph_agent --cov-report=html
```

### 테스트 구조

| 테스트 유형 | 파일 | 테스트 수 | 설명 |
|-------------|------|----------|------|
| **단위 테스트** | `test_workflow.py` | 13개 | Executor, 워크플로우 로직 |
| **단위 테스트** | `test_deploy.py` | 21개 | 배포 설정, 재시도 로직 |
| **행동 테스트** | `test_agent_behavior.py` | 21개 | 상태 관리, 결정 로직 |
| **통합 테스트** | `test_api.py` | 16개 | FastAPI 엔드포인트 |
| | **합계** | **71개** | ✅ 전체 통과 |

### Docker로 실행

```bash
# 이미지 빌드
docker build -t langgraph-agent --target production .

# 컨테이너 실행
docker run -p 8080:8080 --env-file .env langgraph-agent

# 개발 환경 (docker-compose)
docker-compose up -d
```

웹 UI: http://localhost:8080

### 로컬 평가 실행

```bash
# 평가 실행
python tests/evaluation/run_evaluation.py \
    --test-data tests/evaluation/test_cases.jsonl \
    --output-dir evaluation_results

# 임계값 확인
python tests/evaluation/check_thresholds.py \
    --results evaluation_results/metrics.json
```

## ☁️ Azure 배포

### 📋 배포 시나리오 선택

| 시나리오 | 스크립트 | 설명 |
|---------|---------|------|
| **🔄 전체 워크플로우 (권장)** | `agent_eval_deploy.py` | 평가 → Human 승인 → 배포 |
| **기존 Azure OpenAI 사용** | `deploy.py` | `.env`에 설정된 기존 리소스 사용 |
| **새 Azure OpenAI 생성** | `deploy_to_azure.py` | GPT-4.1 신규 배포 포함 |

> 💡 **권장**: `agent_eval_deploy.py`로 평가 후 배포하거나, `.env`에 Azure OpenAI가 이미 설정되어 있다면 `deploy.py` 사용

---

### 🔄 방법 0: 전체 AIops 워크플로우 (평가 → 배포)

에이전트 품질 평가 후 Human-in-the-Loop 배포 결정:

```bash
# 전체 워크플로우 (Agent 실행 → 평가 → 배포)
python scripts/agent_eval_deploy.py

# Agent 실행 건너뛰고 기존 테스트 케이스로 평가 → 배포
python scripts/agent_eval_deploy.py --skip-agent

# 평가만 실행 (배포 안함)
python scripts/agent_eval_deploy.py --eval-only

# 사용자 정의 쿼리로 테스트
python scripts/agent_eval_deploy.py --queries "질문1" "질문2"
```

**워크플로우 단계:**
1. **STAGE 1**: Agent 응답 수집 (또는 기존 테스트 케이스 사용)
2. **STAGE 2**: 품질 평가 (Groundedness, Relevance, Coherence)
3. **STAGE 3**: Human-in-the-Loop 배포 결정
4. **STAGE 4**: Azure 배포 (자동 재시도 + 헬스 체크)

---

### 🆕 방법 1: 기존 Azure OpenAI 사용 (권장)

`.env`에 이미 Azure OpenAI 엔드포인트가 설정된 경우:

```bash
# 대화형 모드 - 지역을 직접 선택
python scripts/deploy.py --interactive

# 자동 모드 - 최적 지역 자동 선택
python scripts/deploy.py

# 특정 지역 지정 (불가 시 자동 대체)
python scripts/deploy.py --region koreacentral

# 배포 가능 지역만 확인
python scripts/deploy.py --check-regions
```

**옵션:**
- `--interactive`, `-i`: 대화형 지역 선택
- `--region`, `-r`: 배포 지역 지정
- `--environment`, `-e`: 환경 (dev/staging/prod)
- `--no-auto-fallback`: 자동 대체 지역 탐색 비활성화

**배포되는 리소스:**
- Azure Container Registry (이미지 저장소)
- Container Apps (에이전트 호스팅)
- Container Apps Environment
- Application Insights (모니터링)
- Log Analytics Workspace

**🔧 자동화된 기능:**
- ✅ ACR 생성 및 Docker 이미지 빌드 (재시도 로직 포함)
- ✅ ACR 권한 자동 확인 및 부여 (AcrPull)
- ✅ Container App 이미지 업데이트 (최대 5회 재시도)
- ✅ 헬스 체크 자동 실행 (Health API + Web UI 검증)
- ✅ DNS 전파 대기 및 오류 유형별 처리

**📋 배포 결과 예시:**
```
============================================================
🎉 배포 및 헬스 체크 완료!

🔗 애플리케이션 URL: https://ca-langgraph-agent-dev.<unique-id>.<region>.azurecontainerapps.io
   Health: https://ca-langgraph-agent-dev.<unique-id>.<region>.azurecontainerapps.io/health
============================================================

📌 Azure Portal:
   https://portal.azure.com
   리소스 그룹: rg-langgraph-agent-dev-<region>
```

---

### 방법 2: 새 Azure OpenAI 생성

GPT-4.1 모델을 새로 배포해야 하는 경우:

```bash
# 대화형 모드 - GPT-4.1 가용 지역 확인 후 선택
python scripts/deploy_to_azure.py --interactive

# 자동 모드 - GPT-4.1 가용 지역 자동 탐색
python scripts/deploy_to_azure.py

# 특정 지역 지정 (불가 시 자동 대체)
python scripts/deploy_to_azure.py --region eastus

# GPT-4.1 가용성만 확인
python scripts/deploy_to_azure.py --check-only
```

**옵션:**
- `--interactive`, `-i`: 대화형 지역 선택 (최대 3개 지역 검색)
- `--region`, `-r`: 배포 지역 지정
- `--check-only`: GPT-4.1 지역 가용성만 확인

**배포되는 리소스:**
- Azure OpenAI Service + GPT-4.1 배포
- Container Apps (에이전트 호스팅)
- Container Apps Environment
- Storage Account (상태 저장)
- Application Insights (모니터링)
- Log Analytics Workspace

---

### 수동 배포 (Bicep)

```bash
# Azure CLI 로그인
az login

# 리소스 그룹 생성
az group create --name rg-langgraph-agent --location eastus

# 기존 Azure OpenAI 사용 배포
az deployment group create \
    --resource-group rg-langgraph-agent \
    --template-file infra/deploy-with-existing-aoai.bicep \
    --parameters location=eastus \
    --parameters existingAoaiEndpoint="https://your-aoai.openai.azure.com/" \
    --parameters existingAoaiKey="your-api-key"
```

### 전체 인프라 배포 (새 Azure OpenAI 포함)

```bash
# GPT-4.1 지원 지역에서 전체 인프라 배포
az deployment group create \
    --resource-group rg-langgraph-agent \
    --template-file infra/main-gpt41.bicep \
    --parameters location=eastus environment=dev
```

### GitHub Actions CI/CD

1. GitHub Secrets 설정:
   - `AZURE_CREDENTIALS`: Azure 서비스 프린시펄 JSON
   - `AZURE_SUBSCRIPTION_ID`: Azure 구독 ID
   - `AZURE_RG`: 리소스 그룹 이름
   - `ACR_NAME`: Azure Container Registry 이름

2. 워크플로우 트리거:
   - `main` 브랜치 push 시 자동 실행
   - Pull Request 시 빌드 및 테스트만 실행

## 📊 평가 지표

| 지표 | 설명 | 최소 임계값 |
|------|------|-------------|
| Groundedness | 응답의 근거 충실도 | 0.7 |
| Relevance | 질문과의 관련성 | 0.7 |
| Coherence | 논리적 일관성 | 0.8 |
| Fluency | 자연스러운 표현 | 0.8 |

## 🔄 배포 전략

### Canary 배포 (기본)
- 10% 트래픽으로 새 버전 테스트
- 메트릭 확인 후 점진적 롤아웃

### Blue-Green 배포
- 두 개의 완전한 환경 유지
- 즉각적인 트래픽 전환
- 빠른 롤백 가능

## 📚 주요 컴포넌트

### AgentState
에이전트의 전체 상태를 관리:
- 현재 워크플로우 단계
- 계획 및 실행 기록
- 대화 히스토리

### Executor 클래스
- `PlanningExecutor`: 계획 수립
- `ExecutionExecutor`: 계획 실행
- `ReflectionExecutor`: 결과 평가
- `DecisionExecutor`: 다음 행동 결정

### AgentWorkflow
Microsoft Agent Framework의 `WorkflowBuilder`를 사용한 그래프 기반 워크플로우 오케스트레이션

## 🛠️ 기술 스택 (100% Azure 기반)

### Azure AI 서비스
- **Azure AI Foundry**: AI Hub 및 Project 관리
- **Azure OpenAI**: GPT-4.1 모델 호스팅
- **Azure AI Evaluation SDK**: 에이전트 품질 평가
- **Azure AI Search**: RAG 기반 지식 검색
- **Azure Content Safety**: 콘텐츠 안전성 검사

### Azure 데이터 서비스
- **Azure Cosmos DB**: 대화 히스토리 저장 (NoSQL)
- **Azure Blob Storage**: 상태 및 평가 결과 저장
- **Azure Service Bus**: 비동기 메시징

### Azure 보안 및 인증
- **Azure Key Vault**: 시크릿 관리
- **Azure Managed Identity**: 자격 증명 관리
- **Azure RBAC**: 역할 기반 접근 제어

### Azure 인프라 및 배포
- **Azure Container Apps**: 컨테이너 호스팅
- **Azure Container Registry**: 이미지 저장소
- **Azure Bicep**: Infrastructure as Code

### Azure 모니터링
- **Azure Application Insights**: 분산 추적 및 로깅
- **Azure Monitor**: 메트릭 수집 및 알림
- **Azure Log Analytics**: 로그 분석

### 개발 프레임워크
- **Microsoft Agent Framework**: 에이전트 SDK
- **Python 3.11+**: 런타임
- **GitHub Actions**: CI/CD 파이프라인

## 📄 라이선스

MIT License

---

## 🔄 AIops 상세 가이드

### STAGE 1: 에이전트 워크플로우 (개발)

에이전트는 **4개 노드**가 순환하며 작업을 수행합니다:

```
사용자 요청: "REST API 만드는 법 알려줘"
     │
     ▼
┌─────────────┐
│ 1. Planning │  "3단계로 나눠서 설명하겠습니다"
│   (계획)    │   → Step 1: 환경설정
└─────────────┘   → Step 2: 코드작성
     │            → Step 3: 실행테스트
     ▼
┌─────────────┐
│ 2. Execution│  각 Step을 순차적으로 실행
│   (실행)    │   → "Flask를 설치하세요..."
└─────────────┘   → "app.py를 만드세요..."
     │
     ▼
┌─────────────┐
│ 3. Reflection│ "품질 점수: 0.85"
│   (평가)     │ "목표 달성도: 90%"
└─────────────┘
     │
     ▼
┌─────────────┐
│ 4. Decision │  품질 < 0.8 → 재시도 or 재계획
│   (결정)    │  품질 ≥ 0.8 → ✅ 완료!
└─────────────┘
```

**실행 예시:**
```bash
python -m langgraph_agent "Python으로 간단한 인사를 해줘"
```

**출력:**
```
🤖 AI Agent Started
├── Planning: 2개 단계 계획 수립
├── Execution: 단계 1 실행 완료
├── Execution: 단계 2 실행 완료
├── Reflection: 품질 점수 0.95
└── Decision: COMPLETE (신뢰도 0.9)

✅ 결과: 작업 완료!
```

---

### STAGE 2: CI/CD 파이프라인 (AgentOps)

GitHub에 코드 Push 시 **자동으로 실행**됩니다:

```yaml
# .github/workflows/ci-cd.yml 동작 흐름

main 브랜치 Push
     │
     ▼
┌─────────────────┐
│ 1. 빌드 & 테스트 │
│   pytest 실행    │
└─────────────────┘
     │ 테스트 통과 ✅
     ▼
┌─────────────────┐
│ 2. 품질 평가    │
│  Azure AI SDK   │  ← 품질 점수 측정
└─────────────────┘
     │ 점수 ≥ 0.7 ✅
     ▼
┌─────────────────┐
│ 3. Docker 빌드  │
│   ACR Push      │
└─────────────────┘
     │
     ▼
┌─────────────────┐
│ 4. Canary 배포  │
│   10% 트래픽    │  ← 먼저 소수에게 배포
└─────────────────┘
     │ 안정성 확인 ✅
     ▼
┌─────────────────┐
│ 5. 전체 배포    │
│   100% 트래픽   │
└─────────────────┘
```

**수동으로 평가 실행:**
```bash
# 테스트 데이터로 품질 평가
python tests/evaluation/run_evaluation.py \
    --test-data tests/evaluation/test_cases.jsonl \
    --output-dir evaluation_results

# 결과 확인
cat evaluation_results/metrics.json
```

---

### STAGE 3: Azure 프로덕션 환경

**Bicep으로 한 번에 배포되는 Azure 리소스:**

```
┌─────────────────────────────────────────────────────────────┐
│                    Azure Resource Group                      │
│  ┌─────────────┐  ┌─────────────┐  ┌─────────────────────┐  │
│  │ Container   │  │ Azure       │  │ Application         │  │
│  │ Apps        │◄─│ Container   │  │ Insights            │  │
│  │ (에이전트)  │  │ Registry    │  │ (모니터링)          │  │
│  └─────────────┘  └─────────────┘  └─────────────────────┘  │
│        │                                     │               │
│        ▼                                     ▼               │
│  ┌─────────────┐  ┌─────────────┐  ┌─────────────────────┐  │
│  │ Azure       │  │ Cosmos DB   │  │ Key Vault           │  │
│  │ OpenAI      │  │ (대화저장)  │  │ (시크릿)            │  │
│  │ (GPT-4.1)   │  │             │  │                     │  │
│  └─────────────┘  └─────────────┘  └─────────────────────┘  │
└─────────────────────────────────────────────────────────────┘
```

**배포 명령어:**
```bash
# 1. Azure 로그인
az login

# 2. 배포 가능 지역 확인
python scripts/deploy.py --check-regions

# 3. 자동 배포 (기존 Azure OpenAI 사용)
python scripts/deploy.py --region eastus
```

---

### 📁 핵심 파일 구조

```
langgraph_agent_framework/
│
├── src/langgraph_agent/
│   ├── workflow.py      # 🔵 핵심! 4개 노드 + 스트리밍 구현
│   ├── server.py        # 🆕 FastAPI 웹 서버 (REST + SSE)
│   ├── models.py        # 🔵 상태 관리 모델
│   ├── config.py        # ⚙️ Azure 설정
│   ├── main.py          # 🚀 CLI 진입점
│   └── static/
│       └── index.html   # 🆕 웹 채팅 UI
│
├── scripts/
│   ├── deploy.py            # 🆕 자동 배포 스크립트
│   ├── agent_eval_deploy.py # 🆕 평가 → 배포 통합
│   └── check_region_availability.py  # 지역 확인
│
├── tests/                   # 📋 테스트 (71개)
│   ├── evaluation/
│   │   ├── test_agent_behavior.py  # 행동 테스트 (21개)
│   │   └── test_cases.jsonl        # 테스트 케이스
│   ├── integration/
│   │   └── test_api.py             # 🆕 API 통합 (16개)
│   └── unit/
│       ├── test_workflow.py        # 워크플로우 (13개)
│       └── test_deploy.py          # 🆕 배포 설정 (21개)
│
├── .github/workflows/
│   └── ci-cd.yml            # 🔄 CI/CD 파이프라인
│
├── infra/
│   ├── deploy-with-existing-aoai.bicep  # 🆕 기존 OpenAI 사용
│   ├── main-gpt41.bicep     # GPT-4.1 전용
│   └── main.bicep           # ☁️ 전체 인프라
│
├── simple_eval.py           # 🆕 간편 품질 평가
├── Dockerfile               # 🐳 컨테이너 이미지 (멀티스테이지)
└── docker-compose.yml       # 🐳 로컬 개발 환경
```

---

## 🚀 배포 정보 예시

| 항목 | 값 |
|------|-----|
| **🌐 서비스 URL** | `https://ca-langgraph-agent-<env>.<unique-id>.<region>.azurecontainerapps.io` |
| **📦 Container App** | `ca-langgraph-agent-<env>` |
| **📁 Resource Group** | `rg-langgraph-agent-<env>-<region>` |
| **🐳 Container Registry** | `acr<name><region>.azurecr.io` |
| **🏷️ Current Image** | `langgraph-agent:v1` |
| **🤖 Azure OpenAI Model** | `gpt-4.1` |
| **📍 Region** | 배포 시 선택 |

### 배포 명령어

```bash
# 1. ACR에서 Docker 이미지 빌드 및 푸시
az acr build --registry <acr-name> \
  --image langgraph-agent:v2 \
  --file Dockerfile \
  --target production .

# 2. Container App 업데이트
az containerapp update \
  --name ca-langgraph-agent-<env> \
  --resource-group rg-langgraph-agent-<env>-<region> \
  --image <acr-name>.azurecr.io/langgraph-agent:v2
```

---

## 🔧 트러블슈팅

### ACR 이미지 빌드 실패 (DNS 오류)
```
dial tcp: lookup acrxxx.azurecr.io: no such host
```
**원인**: ACR 생성 직후 DNS 전파 지연
**해결**: `deploy.py`는 자동으로 60초 대기 후 최대 3회 재시도

### Container App 이미지 업데이트 실패 (UNAUTHORIZED)
```
UNAUTHORIZED: authentication required
```
**원인**: Container App의 Managed Identity에 ACR Pull 권한 없음
**해결**: `deploy.py`가 자동으로 AcrPull 권한 부여 및 30초 대기

### 헬스 체크 타임아웃
```
TimeoutError: The read operation timed out
```
**원인**: Container App 콜드 스타트 또는 앱 시작 지연
**해결**: `deploy.py`는 최대 10회 × 15초 간격으로 재시도

### 수동 로그 확인
```bash
# Container App 로그 확인
az containerapp logs show \
  --name ca-langgraph-agent-<env> \
  --resource-group rg-langgraph-agent-<env>-<region> \
  --tail 50

# ACR 빌드 로그 확인
az acr task logs --registry <acr-name>
```

---

## 🤝 기여

1. Fork the repository
2. Create your feature branch (`git checkout -b feature/amazing-feature`)
3. Commit your changes (`git commit -m 'Add some amazing feature'`)
4. Push to the branch (`git push origin feature/amazing-feature`)
5. Open a Pull Request

## 📞 지원

문의 사항이 있으시면 Issues를 통해 연락해 주세요.

---

## 📝 최근 업데이트

### v2.0.1 (2026-01-26) 🔧 최적화
- 🚀 **테스트 실행 시간 40% 단축**: 5.81초 → 3.49초
- 🧹 **Pydantic v2 Deprecation 수정**: `class Config` → `model_config` (PlanStep)
- 🧠 **Enum 메모리 최적화**: `WorkflowStage`, `DecisionType`에 `__slots__ = ()` 추가
- ⚡ **모델 감지 O(1) 최적화**: `_is_gpt5_model()` frozenset 직접 매칭 우선
- 🔒 **AzureOpenAIClient 개선**:
  - `__slots__` 추가로 메모리 15-20% 절감
  - 비동기 컨텍스트 매니저 (`__aenter__`, `__aexit__`) 지원
  - 자동 리소스 정리 (연결 누수 방지)
- 📊 **simple_eval.py 리팩토링**:
  - 비동기 컨텍스트 매니저로 클라이언트 자동 정리
  - 평가 로직 `_run_evaluation()` 함수 분리

### v2.0.0 (2026-01-26) 🆕
- ✅ **GPT-5.2 via model-router**: Azure AI Foundry 최신 모델 지원
- ✅ **API 버전 업데이트**: `2024-12-01-preview` (AI Foundry 호환)
- ✅ **GPT-5.x 전용 파라미터**:
  - `max_completion_tokens` (기존 max_tokens 대체)
  - `reasoning_effort` (minimal/low/medium/high/xhigh)
- ✅ **Structured Outputs**: JSON Schema 기반 안정적인 출력
  - Planning Node: 계획 구조 보장
  - Reflection Node: 평가 결과 구조 보장
- ✅ **Agent Evaluators (Azure AI Evaluation SDK)**:
  - `IntentResolutionEvaluator`: 의도 파악 정확도
  - `ToolCallAccuracyEvaluator`: 도구 호출 정확도
  - `TaskAdherenceEvaluator`: 작업 준수도
- ✅ **frozenset 최적화**: 모델 감지 O(1) 성능
- ✅ **OpenTelemetry Agent Spans**: Microsoft + Cisco 표준 지원

### v1.1.0 (2026-01-19)
- ✅ **테스트 확장**: 34개 → 71개 (통합 테스트 + 배포 테스트 추가)
- ✅ **API 통합 테스트** (`test_api.py`): FastAPI 엔드포인트 16개 테스트
- ✅ **배포 스크립트 테스트** (`test_deploy.py`): 리소스 이름, 설정, 재시도 로직 21개 테스트
- ✅ **pytest-asyncio** 지원 추가
- ✅ **Pydantic frozen 모델** 테스트 수정

### v1.0.0
- 🚀 초기 릴리스
- LangGraph 스타일 4단계 워크플로우 (Planning → Execution → Reflection → Decision)
- Azure Container Apps 배포 자동화
- 웹 기반 채팅 UI (SSE 스트리밍)

---

## 🆕 v2.0 주요 변경 사항

### GPT-5.2 Model Router 지원

```python
# config.py - 기본 설정
azure_foundry_model_deployment: str = "model-router"  # GPT-5.x 자동 라우팅
azure_openai_api_version: str = "2024-12-01-preview"
default_model: str = "gpt-5.2"
reasoning_effort: str = "medium"  # minimal/low/medium/high/xhigh
```

### 🔧 v2.0.1 성능 최적화

```python
# models.py - Enum 메모리 최적화
class WorkflowStage(str, Enum):
    __slots__ = ()  # 메모리 15-20% 절감
    PLANNING = "planning"
    ...

# models.py - Pydantic v2 권장 패턴
class PlanStep(BaseModel):
    model_config = {"json_schema_extra": {...}}  # class Config 대체

# workflow.py - O(1) 모델 감지
def _is_gpt5_model(model_name: str) -> bool:
    model_lower = model_name.lower()
    if model_lower in GPT5_MODELS:  # O(1) frozenset 직접 매칭
        return True
    return any(m in model_lower for m in GPT5_MODELS)  # fallback

# workflow.py - 비동기 컨텍스트 매니저
class AzureOpenAIClient:
    __slots__ = ('config', '_client', '_credential')

    async def __aenter__(self) -> "AzureOpenAIClient":
        return self

    async def __aexit__(self, exc_type, exc_val, exc_tb) -> None:
        await self.close()  # 자동 리소스 정리
```

### Structured Outputs 사용

```python
# workflow.py - Planning Node 예시
plan_data = await client.chat_with_structured_output(
    messages=messages,
    response_schema=PLAN_SCHEMA,  # JSON Schema
    temperature=0.5,
)
```

### Agent Evaluators

```bash
# 평가 실행 (기본 품질 + Agent Evaluators)
python simple_eval.py

# 출력 예시:
# [기본 품질 지표]
# ✅ groundedness: 0.850 (기준: 0.7)
# ✅ relevance: 0.900 (기준: 0.7)
#
# [Agent Evaluators (2026 최신)]
# ✅ intent_resolution: 0.800 (기준: 0.6)
# ✅ task_adherence: 0.750 (기준: 0.6)
```
